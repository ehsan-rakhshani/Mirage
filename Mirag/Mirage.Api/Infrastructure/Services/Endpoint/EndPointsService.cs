...
public class EndPointsService
{
    ...
    public async Task<IEnumerable<MyRouteDto>> GetList()
    {
        await Task.Delay(2000);
        var result = new List<MyRouteDto>();
    ...
    private ModelBindingType GetBindingSource(ParameterInfo parameter)
    {
        ...
        if (attributes.OfType<FromQueryAttribute>().Any())
            return ModelBindingType.Query;

        if (attributes.OfType<FromBodyAttribute>().Any())
            return ModelBindingType.Body;

        if (attributes.OfType<FromRouteAttribute>().Any())
            return ModelBindingType.Route;

        if (attributes.OfType<FromHeaderAttribute>().Any())
            return ModelBindingType.Header;

        if (attributes.OfType<FromFormAttribute>().Any())
            return ModelBindingType.Form;
